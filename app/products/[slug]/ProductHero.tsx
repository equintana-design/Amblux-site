...
export function ProductHero({ page }: { page: ProductPage }) {
  ...
  return (
    <section className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-12 px-6 py-14 lg:grid-cols-[1.05fr_0.95fr] lg:gap-20 lg:py-20">
      <div className="relative aspect-square w-full overflow-hidden rounded-2xl border border-border bg-background">
        {heroImage ? (
          <Image src={heroImage} alt={page.name} fill className="object-cover" sizes="(min-width: 1024px) 45vw, 100vw" />
        ) : null}
      </div>
      ...
      <div>
        ...
        {axes.length > 0 ? (
          <div className="mt-6 rounded-2xl border border-border bg-background p-5">
            ...
            <p className="mt-4 flex flex-wrap items-baseline justify-between gap-2 border-t border-border pt-4 text-sm">
        ...
        <button
          type="button"
          className="mt-6 inline-flex items-center gap-2 rounded-full bg-accent-soft px-6 py-3 text-sm font-semibold text-foreground transition-colors hover:bg-accent-strong hover:text-white"
        >
          Add to test project <span aria-hidden="true">+</span>
        </button>

        <div className="mt-4 flex flex-wrap gap-4 text-sm font-medium text-accent-strong">
        ...
        <p className="mt-2 text-xs text-muted">Save the branded product sheet as a PDF, then attach it to the prepared email.</p>
        <p className="mt-6 text-xs text-muted">Product information and image sourced from the AMBLUX master product database and model folder.</p>
      </div>
    </section>
  );
}
